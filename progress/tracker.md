# Progress tracker

Updated by Claude after every check-in (10:00 and 22:00 IST). Status values: `not started`, `in progress`, `done`, `partial` (carried over), `skipped`.
Quiz score: each answer earns 2 (complete and precise), 1 (partly right or vague) or 0 (wrong or blank); the score shown is points earned out of points possible, as a percentage.

| Day | Date       | Focus                                               | Status      | Hours | AM quiz | PM quiz | Notes                                                              |
| --- | ---------- | --------------------------------------------------- | ----------- | ----- | ------- | ------- | ------------------------------------------------------------------ |
| 0   | Thu 24 Sep | F0: lab machine setup                               | in progress |       |         |         | VM running; hit missing `rh` (clone/build) and cgo/gcc build error |
| 1   | Fri 25 Sep | F1 §1–3: processes, syscalls, fds                   | not started |       |         |         |                                                                    |
| 2   | Sat 26 Sep | F1 §4–8: signals, users, mounts, /proc, systemd     | not started |       |         |         |                                                                    |
| 3   | Sun 27 Sep | F2: networking fundamentals                         | not started |       |         |         |                                                                    |
| 4   | Mon 28 Sep | F3: Go for infrastructure                           | not started |       |         |         |                                                                    |
| 5   | Tue 29 Sep | F4: Docker & Compose + your own 60-line runtime     | not started |       |         |         |                                                                    |
| 6   | Wed 30 Sep | Module 1: namespaces                                | not started |       |         |         |                                                                    |
| 7   | Thu 01 Oct | Module 2: filesystems (ends early)                  | not started |       |         |         |                                                                    |
| 8   | Fri 02 Oct | Modules 3–4: cgroups, security                      | not started |       |         |         |                                                                    |
| 9   | Sat 03 Oct | Module 5: container networking                      | not started |       |         |         |                                                                    |
| 10  | Sun 04 Oct | Module 6: images and registries                     | not started |       |         |         |                                                                    |
| 11  | Mon 05 Oct | Module 7: builds                                    | not started |       |         |         |                                                                    |
| 12  | Tue 06 Oct | F5 + F6 §1–4: Kubernetes, distributed systems       | not started |       |         |         |                                                                    |
| 13  | Wed 07 Oct | Module 8: provisioning engine, part 1               | not started |       |         |         |                                                                    |
| 14  | Thu 08 Oct | Module 8 part 2 + first engine feature (ends early) | not started |       |         |         |                                                                    |
| 15  | Fri 09 Oct | Module 9 + F6 §5–7: scale                           | not started |       |         |         |                                                                    |
| 16  | Sat 10 Oct | Build day: seccomp or multi-node controller         | not started |       |         |         |                                                                    |
| 17  | Sun 11 Oct | Security, production code reading, module 11        | not started |       |         |         |                                                                    |
| 18  | Mon 12 Oct | Gaps and depth                                      | not started |       |         |         |                                                                    |
| 19  | Tue 13 Oct | Module 10: question bank + mock #1                  | not started |       |         |         |                                                                    |
| 20  | Wed 14 Oct | Mock #2 + wrap-up                                   | not started |       |         |         |                                                                    |

## Totals

| Metric               | Value  |
| -------------------- | ------ |
| Days completed       | 0 / 20 |
| Hours studied        | 0      |
| Average quiz score   | –      |
| Exercises finished   | 0      |
| Mock interviews done | 0 / 2  |
